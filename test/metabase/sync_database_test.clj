...
(defn- x []
  (tt/with-temp Database [db {:engine :sync-test}]
    ...
    ;; on resync of a database, such as adding tables that already exist or duplicating fields
    (sync-database! db)
    (for [field (:fields (toucan.hydrate/hydrate (db/select-one Table, :db_id (u/get-id db), :name "movie") :fields))]
      (select-keys field [:id :name :special_type :fk_target_field_id]))))
...
;; Make sure that if a Field's cardinality passes `metabase.sync.interface/low-cardinality-threshold` (currently 300)
